...
- Los coeficientes más significativos son:
  ...
  - La variable continente ya no es significativa a la hora de explicar el precio de los jugadores, pero se observa como se invierte la relación con los jugadores europeas donde los africanos son mas valorados en este modelo que en el clasico.
...
- El R² ajustado es 0.4701, indicando que el modelo explica aproximadamente el 47% de la variabilidad, no mostrando cambios con respecto al 47% del modelo clásico
...
modelo_multiple_lmrob_hampel_2 <- lmrob(formula = precio ~ Gls + Age + I(Age^2) + Ast + 
                                         continente + current_club_domestic_competition_id, 
                                       data=train_data,
                                       psi = "hampel")
...
modelo_multiple_lmrob_ggw_2 <- lmrob(formula = precio ~ Gls + Age + I(Age^2) + Ast + 
                                         continente + current_club_domestic_competition_id, 
                                       data=train_data,
                                       psi = "ggw")
...
modelos <- list(multiple_1 = modelo_clasico_multiple_1, 
                ...
                robusto_6_1 = modelo_multiple_lmrob_hampel_2,
                robusto_6 = modelo_multiple_lmrob_hampel,
                robusto_7_1 = modelo_multiple_lmrob_ggw_2,
                robusto_7 = modelo_multiple_lmrob_ggw)

lista_predicciones_training = map(.x = modelos, .f = augment, newdata = train_data) 
...
metricas4_1_test = lista_predicciones_testing$robusto_3_1 %>%  
  metrics(truth=precio, estimate=.fitted) %>%
  mutate(.estimate=round(.estimate, 4))
...
metricas5_1_test = lista_predicciones_testing$robusto_4_1 %>%  
  metrics(truth=precio, estimate=.fitted) %>%
  mutate(.estimate=round(.estimate, 4))
...
metricas6_1_test = lista_predicciones_testing$robusto_5_1 %>%  
  metrics(truth=precio, estimate=.fitted) %>%
  mutate(.estimate=round(.estimate, 4))
...
metricas7_1_test = lista_predicciones_testing$robusto_6_1 %>%  
  metrics(truth=precio, estimate=.fitted) %>%
  mutate(.estimate=round(.estimate, 4))
...
metricas8_1_test = lista_predicciones_testing$robusto_7_1 %>%  
  metrics(truth=precio, estimate=.fitted) %>%
  mutate(.estimate=round(.estimate, 4))
...
metricas_test <- rbind(metricas1_test[c(1,3),], metricas2_test[c(1,3),])
metricas_test <- rbind(metricas_test, metricas_robusto_test[c(1,3),])
metricas_test <- rbind(metricas_test, metricas3_test[c(1,3),])
metricas_test <- rbind(metricas_test, metricas4_1_test[c(1,3),])
metricas_test <- rbind(metricas_test, metricas4_test[c(1,3),])
metricas_test <- rbind(metricas_test, metricas5_1_test[c(1,3),])
metricas_test <- rbind(metricas_test, metricas5_test[c(1,3),])
metricas_test <- rbind(metricas_test, metricas6_1_test[c(1,3),])
metricas_test <- rbind(metricas_test, metricas6_test[c(1,3),])
metricas_test <- rbind(metricas_test, metricas7_1_test[c(1,3),])
metricas_test <- rbind(metricas_test, metricas7_test[c(1,3),])
metricas_test <- rbind(metricas_test, metricas8_1_test[c(1,3),])
metricas_test <- rbind(metricas_test, metricas8_test[c(1,3),])

modelitos <- c(rep("Clasico - Precio",2),
               rep("Clasico - log(Precio)",2),
               rep("Robusto - Precio - psi = bisquare",2),
               rep("Robusto - log(Precio) - psi = bisquare",2),
               rep("Robusto - Precio - psi = lqq",2),
               rep("Robusto - log(Precio) - psi = lqq",2),
               rep("Robusto - Precio - psi = welsh",2), 
               rep("Robusto - log(Precio) - psi = welsh",2), 
               rep("Robusto - Precio - psi = optimal",2),
               rep("Robusto - log(Precio) - psi = optimal",2),
               rep("Robusto - Precio - psi = hampel",2),
               rep("Robusto - log(Precio) - psi = hampel",2),
               rep("Robusto - Precio - psi = ggw",2),
               rep("Robusto - log(Precio) - psi = ggw",2))


metricas_test <- cbind(modelitos, metricas_test)
kable(metricas_test[metricas_test$.metric == "rmse",])
kable(metricas_test[metricas_test$.metric == "mae",])

modelitos <- c(rep("Clasico - Precio",1),
               rep("Clasico - log(Precio)",1),
               rep("Robusto - Precio - psi = bisquare",1),
               rep("Robusto - log(Precio) - psi = bisquare",1),
               rep("Robusto - Precio - psi = lqq",1),
               rep("Robusto - log(Precio) - psi = lqq",1),
               rep("Robusto - Precio - psi = welsh",1), 
               rep("Robusto - log(Precio) - psi = welsh",1), 
               rep("Robusto - Precio - psi = optimal",1),
               rep("Robusto - log(Precio) - psi = optimal",1),
               rep("Robusto - Precio - psi = hampel",1),
               rep("Robusto - log(Precio) - psi = hampel",1),
               rep("Robusto - Precio - psi = ggw",1),
               rep("Robusto - log(Precio) - psi = ggw",1))

metricas1_train = lista_predicciones_training$multiple_1 %>%  
  metrics(truth=precio, estimate=.fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas2_train = lista_predicciones_training$multiple_2 %>%  
  mutate(exp_fitted= exp(.fitted)) %>% 
  metrics(truth=precio, estimate=exp_fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas3_train = lista_predicciones_training$robusto_2 %>% 
  mutate(exp_fitted= exp(.fitted)) %>%
  metrics(truth=precio, estimate=exp_fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas4_1_train = lista_predicciones_training$robusto_3_1 %>%  
  metrics(truth=precio, estimate=.fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas4_train = lista_predicciones_training$robusto_3 %>%  
  mutate(exp_fitted= exp(.fitted)) %>% 
  metrics(truth=precio, estimate=exp_fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas5_1_train = lista_predicciones_training$robusto_4_1 %>%  
  metrics(truth=precio, estimate=.fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas5_train = lista_predicciones_training$robusto_4 %>%  
  mutate(exp_fitted= exp(.fitted)) %>% 
  metrics(truth=precio, estimate=exp_fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas6_1_train = lista_predicciones_training$robusto_5_1 %>%  
  metrics(truth=precio, estimate=.fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas6_train = lista_predicciones_training$robusto_5 %>%  
  mutate(exp_fitted= exp(.fitted)) %>% 
  metrics(truth=precio, estimate=exp_fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas7_1_train = lista_predicciones_training$robusto_6_1 %>%  
  metrics(truth=precio, estimate=.fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas7_train = lista_predicciones_training$robusto_6 %>%  
  mutate(exp_fitted= exp(.fitted)) %>% 
  metrics(truth=precio, estimate=exp_fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas8_1_train = lista_predicciones_training$robusto_7_1 %>%  
  metrics(truth=precio, estimate=.fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas8_train = lista_predicciones_training$robusto_7 %>%  
  mutate(exp_fitted= exp(.fitted)) %>% 
  metrics(truth=precio, estimate=exp_fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas9_train = lista_predicciones_training$robusto_1 %>%
  metrics(truth=precio, estimate=.fitted) %>%
  mutate(.estimate=round(.estimate, 4))

metricas_train <- rbind(metricas1_train[2,], metricas2_train[2,])
metricas_train <- rbind(metricas_train, metricas9_train[2,])
metricas_train <- rbind(metricas_train, metricas3_train[2,])
metricas_train <- rbind(metricas_train, metricas4_1_train[2,])
metricas_train <- rbind(metricas_train, metricas4_train[2,])
metricas_train <- rbind(metricas_train, metricas5_1_train[2,])
metricas_train <- rbind(metricas_train, metricas5_train[2,])
metricas_train <- rbind(metricas_train, metricas6_1_train[2,])
metricas_train <- rbind(metricas_train, metricas6_train[2,])
metricas_train <- rbind(metricas_train, metricas7_1_train[2,])
metricas_train <- rbind(metricas_train, metricas7_train[2,])
metricas_train <- rbind(metricas_train, metricas8_1_train[2,])
metricas_train <- rbind(metricas_train, metricas8_train[2,])
metricas_train <- cbind(modelitos, metricas_train)
kable(metricas_train)
```
...
Esta diferencia refleja una evolución en la comprensión del valor creativo en el fútbol moderno, donde la capacidad de generar oportunidades se premia por encima de la finalización. Este patrón sugiere una sofisticación creciente en la evaluación del talento.
