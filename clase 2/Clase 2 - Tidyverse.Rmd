...
library(tidyverse)
#library(lubridate) # para trabajar con fechas, ya está integrado a tidyverse
```
...
El caracter principal para utilizar este paquete es ```%>%``` , _pipe_ (de tubería). ("Ctrl+Shif+M" pone el pipe directamente)
...
#AND: Acá puedo usar "," o &, son equivalentes
```
...
Si quiero crear mas variables dentro de summarise separo por coma cada una.
