...
m_cor <- datos %>% select(where(is.numeric)) %>% cor()
corrplot(m_cor,
         method="circle",
         type = "upper",
         addCoef.col = "black",
...
#Grafico de correlaciones con apretura por sexo
grafico2 %>% 
...
rmse(data=pred_modelo_propio1, truth = salario_horario, estimate=exp_fitter)
...
rmse(data=pred_modelo_propio2, truth = salario_horario, estimate=exp_fited)
